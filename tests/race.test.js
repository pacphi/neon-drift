...
import { createRace, passCheckpoint, standings, isFinished, timedOut, currentLapTime } from '../src/game/race.js';
...
describe('race', () => {
  ...
  it('records per-lap and best-lap times from race.time', () => {
    ...
    r.time = 20; r = passCheckpoint(r, 'p', 0); // lap 1 took 20s
    r.time = 35; r = passCheckpoint(r, 'p', 0); // lap 2 took 15s
  ...
  it('currentLapTime measures elapsed time on the in-progress lap', () => {
    let r = createRace([{ id: 'p' }], { checkpointCount: 1, laps: 3 });
    r.time = 12; r = passCheckpoint(r, 'p', 0); // lap done at t=12
    r.time = 18;
  ...
  it('timedOut true once race.time reaches maxTime', () => {
    ...
    r.time = 99; expect(timedOut(r)).toBe(false);
    r.time = 100; expect(timedOut(r)).toBe(true);
  });

  it('finished racers rank ahead of unfinished, by finish time', () => {
    let r = createRace([{ id: 'a' }, { id: 'b' }, { id: 'c' }], { checkpointCount: 1, laps: 1 });
    r.time = 30; r = passCheckpoint(r, 'b', 0); // b finishes at 30
    r.time = 25; r = passCheckpoint(r, 'a', 0); // a finishes at 25 (faster)
    // c never finishes
    const order = standings(r).map(s => s.id);
