...
> **Status:** This is the original task-by-task build plan and is kept for
> history. It has been **executed and superseded** — for the current as-built
> state (gameplay, physics tuning, pnpm/Node 26 toolchain, CI) see
> [`../specs/2026-05-29-neon-drift-design.md`](../specs/2026-05-29-neon-drift-design.md)
> and the repo `README.md`. Notable deviations since this plan: `buildTrackMesh`
> moved to `render/trackMesh.js` (keeping `track.js` pure); typed item shapes +
> start/finish line + checkerboard final lap; 3-2-1-GO countdown; per-track laps
> (3/5/8) with lap/best/total timing and a max-race-time timeout; switched from
> npm to **pnpm 11 / Node 26**; upgraded to Three 0.184 / Vite 8 / Vitest 4 and
> added typecheck/lint/format + GitHub Actions CI + Dependabot.
...
export function passCheckpoint(race, id, cp) {
  ...
  r.cpHits++;
  if (cp === 0) {
    // completed a full loop back to start/finish
    r.lap++;
    if (r.lap >= race.laps) {
      r.finished = true;
...
export function createInput(target = window, canvas = document.body) {
  ...
  canvas.addEventListener('mousedown', (e) => {
    if (e.button === 0) mouseThrottle = 1;
    if (e.button === 2) mouseItem = true;
  });
  canvas.addEventListener('mouseup', (e) => {
    if (e.button === 0) mouseThrottle = 0;
  });
...
export function createScene(container) {
  ...
  const dir = new THREE.DirectionalLight(0xff66ff, 0.8);
...
export function trackSelect() {
  return new Promise((res) => {
    const cards = TRACKS.map(
      (t, i) =>
        `<button class="tk" data-i="${i}" style="${neonBtn};border-color:#${t.color.toString(16).padStart(6, '0')};">${t.name}</button>`,
    ).join('');
    const el = screen(
    ...
    el.querySelectorAll('.tk').forEach(
      (b) =>
        (b.onclick = () => {
          el.remove();
          res(Number(b.dataset.i));
        }),
    );
  });
}
...
export function resultsScreen(order) {
  return new Promise((res) => {
    const rows = order
      .map(
        (r, i) =>
          `<div style="font-size:24px;color:${i === 0 ? '#aaff00' : '#18f0ff'};">${i + 1}. ${r.name}${r.finishTime != null ? '  ' + r.finishTime.toFixed(1) + 's' : ''}</div>`,
      )
      .join('');
    const el = screen(
...
export function createAudio() {
  ...
  return {
    ...
    boost() {
      blip(220, 0.3, 'sawtooth', 0.18);
    },
    hit() {
      blip(120, 0.2, 'square', 0.2);
    },
    startEngine() {
      const c = ensure();
      ...
      g.gain.value = 0.04;
      engine.connect(g).connect(c.destination);
      engine.start();
      engine._g = g;
    },
    setRpm(speed01) {
      if (engine) engine.frequency.value = 60 + speed01 * 140;
    },
    stopEngine() {
      engine?.stop();
      engine = null;
...
export function createWorld(scene, track) {
  ...
  const race = createRace(
    karts.map((k) => ({ id: k.id })),
    { checkpointCount: 12, laps: RACE.laps },
  );
  ...
  function offTrack(kart) {
    ...
    for (const w of built.waypoints) {
      const d = (w.x - kart.state.pos.x) ** 2 + (w.z - kart.state.pos.z) ** 2;
      if (d < bd) bd = d;
    }
  ...
  function update(dt, playerInput, audio) {
    ...
    karts.forEach((k, i) => {
      ...
      if (input.useItem && k.item) {
        ...
        k.effects = applyItem(k.effects, k.item, {
    ...
    boxes.forEach((b) => {
      ...
      for (const k of karts)
        if (Math.hypot(k.state.pos.x - b.x, k.state.pos.z - b.z) < 3 && !k.item) {
          k.item = rollItem();
          b.active = false;
    ...
    hazards.forEach((h) => {
      for (const k of karts)
        if (Math.hypot(k.state.pos.x - h.x, k.state.pos.z - h.z) < 2.5)
          k.state.spinT = Math.max(k.state.spinT || 0, 0.6);
    });
...
const loop = createLoop(
  (dt) => {
    ...
    hud.update({
      lap: world.race.racers.player.lap,
      laps: RACE.laps,
