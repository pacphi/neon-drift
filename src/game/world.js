...
import { createRace, passCheckpoint, standings, allFinished, isFinished, timedOut } from './race.js';
...
export function createWorld(scene, track) {
  ...
  const race = createRace(karts.map(k => ({ id: k.id })), { checkpointCount: 12, laps, maxTime });
  ...
  [Math.floor(N * 0.18), Math.floor(N * 0.45), Math.floor(N * 0.72)].forEach(wi => {
    ...
    const rx = w.tz, rz = -w.tx; // unit right vector
    [-0.5, 0, 0.5].forEach((frac, i) => {
      ...
      m.position.set(x, 1.9, z); scene.add(m);
  ...
  function makeStartLine(checker) {
    ...
    const rx = s.tz, rz = -s.tx;            // right vector
    const tx = s.tx, tz = s.tz;             // forward vector
    const ang = Math.atan2(s.tx, s.tz);     // start heading (aligns cell x→right, z→forward)
    const group = new THREE.Group();
    const W = built.halfWidth * 2;
    const cols = 14, cell = W / cols, rows = checker ? 2 : 1;
  ...
  const startLineNormal = makeStartLine(false); scene.add(startLineNormal);
  const startLineFinal = makeStartLine(true); startLineFinal.visible = false; scene.add(startLineFinal);
  ...
  function nearestWaypoint(pos) {
    let best = built.waypoints[0], bd = Infinity;
  ...
  function containToTrack(kart) {
    ...
    if (dist > limit) {
      ...
      if (vn > 0) { kart.state.vel.x -= vn * nx; kart.state.vel.z -= vn * nz; }
  ...
  function update(dt, playerInput, audio) {
    ...
    karts.forEach((k, i) => {
      ...
      if (input.useItem && k.item) {
        ...
        k.effects = applyItem(k.effects, used, { self: { pos: k.state.pos, heading: k.state.heading }, rivals });
        if (used === 'oil') {
          ...
          m.rotation.x = -Math.PI / 2; m.position.set(h.x, 0.12, h.z); scene.add(m);
          hazards.push({ mesh: m, x: h.x, z: h.z, t: h.t });
        }
        if (k.isPlayer) { used === 'boost' ? audio?.boost?.() : audio?.hit?.(); }
        k.item = null;
      }
      ...
      k.effects.spun.forEach(id => {
        ...
        if (t && !(t.state.spinCD > 0)) { t.state.spinT = 0.7; t.state.spinCD = 3; }
      });
      ...
      if (k.state.spinT > 0) { k.state.heading += dt * 9; k.state.spinT -= dt; }
    ...
    for (let a = 0; a < karts.length; a++) {
      for (let b = a + 1; b < karts.length; b++) {
        const ka = karts[a].state, kb = karts[b].state;
        const dx = kb.pos.x - ka.pos.x, dz = kb.pos.z - ka.pos.z;
        const d = Math.hypot(dx, dz) || 0.001;
        const minD = 4;
        if (d < minD) {
          const nx = dx / d, nz = dz / d, push = (minD - d) / 2;
          ka.pos.x -= nx * push; ka.pos.z -= nz * push;
          kb.pos.x += nx * push; kb.pos.z += nz * push;
          const rel = (kb.vel.x - ka.vel.x) * nx + (kb.vel.z - ka.vel.z) * nz;
          if (rel < 0) {
            const imp = rel * 0.6;
            ka.vel.x += nx * imp; ka.vel.z += nz * imp;
            kb.vel.x -= nx * imp; kb.vel.z -= nz * imp;
          }
        }
      }
    }
    ...
    boxes.forEach(b => {
      ...
      for (const k of karts) if (Math.hypot(k.state.pos.x - b.x, k.state.pos.z - b.z) < 5 && !k.item) {
        k.item = b.type; b.active = false; b.mesh.visible = false; b.cooldown = 5;
    ...
    hazards.forEach(h => {
      for (const k of karts) {
        if (Math.hypot(k.state.pos.x - h.x, k.state.pos.z - h.z) < 2.5 && !(k.state.spinCD > 0)) {
          k.state.spinT = 0.7; k.state.spinCD = 3;
        }
      }
    });
